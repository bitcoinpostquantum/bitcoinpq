...
License:	MIT
URL:		https://bitcoinpq.org/
Source0:	https://bitcoinpq.org/download/bitcoinpq-core-%{version}/bitcoinpq-%{version}.tar.gz
...
%description
BitcoinPQ is a digital cryptographic currency that uses peer-to-peer technology to
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19941
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19940
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 29941
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19940
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 19941
	%{_sbindir}/semanage port -d -p tcp 19940
	%{_sbindir}/semanage port -d -p tcp 29941
	%{_sbindir}/semanage port -d -p tcp 29940
